 ...
 * taken, and creates a new row in the accounts table with a hashed password.
...
if ($email != NULL && $password != NULL && $phone != NULL && $address != NULL) {
    ...
    $cmd = "SELECT * FROM accounts WHERE phonenumber=?";
    $stmt = $dbh->prepare($cmd);
    ...
    $cmd = "INSERT INTO accounts (password, email, phonenumber, address) values (?, ?, ?, ?)";
    $stmt = $dbh->prepare($cmd);
